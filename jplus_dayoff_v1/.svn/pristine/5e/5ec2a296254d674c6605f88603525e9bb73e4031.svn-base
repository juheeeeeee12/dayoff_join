...
window.createSEditor2 = function(elIRField, htParams, elSeAppContainer){
	...
	if(htParams.bSkipXssFilter !== true){
		...
		oEditor.registerPlugin({
			_rxFilter:/<\/*(?:applet|b(?:ase|gsound|link)|embed|frame(?:set)?|i(?:frame|layer)|l(?:ayer|ink)|meta|object|s(?:cript|tyle)|title|xml|textarea|script|javascript|vbscript|background)[^>]*?>/gi,
