...
<form id="form" name="form" method="post" onsubmit="return false;">
	...
	<div class="row">
		...
		<div class="col-6 menuIconDiv">
			<div class="card">
	    		<div class="card-body">
	    			<h4 class="card-title">메뉴 아이콘</h4>
	    			<input type="text" name="menuIcon" class="form-control form-control-line" value="<c:out value='${result.MENU_ICON}'/>" maxlength="300"/>
	    		</div>
	    	</div>
	    </div>
	    ...
	    <div class="col-6">
	    	<div class="card">
	    		<div class="card-body">
					...
					<input type="text" name="menuNm" class="form-control form-control-line reqIpt" data-label="메뉴명" value="<c:out value='${result.MENU_NM}'/>" maxlength="30"/>
				</div>
			</div>
		</div>
		<div class="col-6">
	    ...
	    <div class="col-12">
	    	<div class="card">
	    		<div class="card-body">
					...
					<input type="text" name="menuUrl" class="form-control form-control-line reqIpt iptMenuCntnts" data-label="URL" value="<c:out value='${result.MENU_URL}'/>" maxlength="300"/>
...
function fn_chgMenuDepth(){
	...
	if(menuDepth != '1'){ // 중메뉴
		...
		$.ajax({
			type : "POST",
			url : "/cms/system/selectCmsParentMenuListAjax.do",
			traditional : true,
			dataType : 'json',
			data : {
				menuDepth : menuDepth
...
function fn_regist(){
	...
	if(confirm("저장하시겠습니까?")) {
		$.ajax({
			type : "POST",
			url : "/cms/system/saveCmsMenuInfoAjax.do",
			traditional : true,
			dataType : 'json',
			data : $("#form").serialize(),
			success : function(data) {
...
function fn_delete(){
	if(confirm("삭제하시겠습니까?")) {
		$.ajax({
			type : "POST",
			url : "/cms/system/deleteCmsMenuInfoAjax.do",
			traditional : true,
			dataType : 'json',
			data : $("#form").serialize(),
			success : function(data) {
...
function fn_goList(){
	sbmit("form", "/cms/system/mngCmsMenuList.do");
}
</script>
<style>
