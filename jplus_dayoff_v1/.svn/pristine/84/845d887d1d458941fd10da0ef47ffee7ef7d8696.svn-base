...
<div class="container-fluid">
    <form id="form" name="form" method="POST">
		...
		<input type="hidden" id="popupSeq" name="popupSeq" value="<c:out value='${popupSeq}'/>"/>
				            ...
				            <table id="sortingTbl" class="table table-hover contact-list footable-loaded footable user-table" data-page-size="7">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="footable-sortable">팝업 제목<span class="footable-sort-indicator"></span></th>
                                        <th class="center footable-sortable">팝업 구분<span class="footable-sort-indicator"></span></th>
                                ...
                                <tbody>
									<c:if test="${not empty resultList}">
				                		<c:forEach items="${resultList}" var="list" varStatus="status">
				                			<tr id="<c:out value='${list.POPUP_SEQ}'/>" class="showYn_<c:out value='${list.SHOW_YN}'/>">
						                        <td class="num center"><c:out value='${status.count}'/></td>
						                        <td class="listImg"><img src="<c:out value='${list.POPUP_IMG_FILE_PATH_NM}'/>" <c:if test="${empty list.POPUP_IMG_FILE_PATH_NM}">style="opacity: 0;"</c:if>></td>
						                        <td>
						                        	<a href="javascript:fn_detail('Y', '<c:out value='${list.POPUP_SEQ}'/>');"><c:out value='${list.POPUP_TITLE}'/></a>
						                        </td>
						                        <td class="center"><c:out value='${list.POPUP_GB_CD_NM}'/></td>
...
function fn_detail(isModify, popupSeq){
	$("#isModify").val(isModify);
	if(isModify == 'Y') $("#popupSeq").val(popupSeq);
	sbmit("form", "/cms/visual/mngPopupWrite.do");
}
...
function fn_chgOrder(t, gb) {
	...
	if(reOrderArr.length > 0) {
		$.ajax({
			type : "POST",
			url : "/cms/visual/reOrderPopupOrderAjax.do",
			traditional : true,
			dataType : 'json',
			data : {
