							...
							<div class="user_search">
								...
								<div class="ipt_group">
									<select class="resetVal form-control custom-select" id="schGb" name="schGb" style="width: 20%;">
										<option value="boardNm" <c:if test="${schGb eq 'boardNm'}">selected</c:if>>게시판명</option>
									</select>
								...
								<table id="demo-foo-addrow" class="table table-hover contact-list footable-loaded footable user-table" data-page-size="10">
									...
									<thead>
										<tr class="center">
											...
											<th class="footable-sortable">게시판 스타일<span class="footable-sort-indicator"></span></th>
											<th class="footable-sortable">게시판명<span class="footable-sort-indicator"></span></th>
									...
									<tbody>
					                	<c:if test="${not empty resultList}">
					                		<c:forEach items="${resultList}" var="list" varStatus="status">
					                			<tr>
							                        ...
							                        <td class="center"><c:out value='${list.BOARD_TYPE_NM}'/></td>
							                        <td class="left"><a href="javascript:fn_detail('Y', '<c:out value='${list.BOARD_SEQ}'/>');"><c:out value='${list.BOARD_NM}'/></a></td>
					                	...
					                	<c:if test="${empty resultList}">
					                		<tr>
					                        <td class="none no-data" colspan="20">등록된 게시판이 없습니다.</td>
					                    </tr>
					                	</c:if>
									</tbody>
									<tfoot>
...
function fn_detail(isModify, boardSeq){
	$("#isModify").val(isModify);
	if(isModify == 'Y') $("#boardSeq").val(boardSeq);
	sbmit("form", "/cms/board/mngBoardWrite.do");
}
