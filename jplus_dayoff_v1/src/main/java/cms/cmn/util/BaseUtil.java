...
protected static Logger logger = LogManager.getLogger(cms.cmn.util.BaseUtil.class);
	...
	private static String getRandomValue(Random random) {
		...
		if (type == 0) {
			...
			if(randomInt >= 0) {
				result = String.valueOf(charArr[randomInt]);
			}
		} else {
	...
	public static String createNewPassword() throws NoSuchAlgorithmException {
		Random random = SecureRandom.getInstance("SHA1PRNG");
		StringBuffer sb = new StringBuffer();
		sb.append(getRandomValue(random)).append(getRandomValue(random)).append(getRandomValue(random))
	...
	public static String splitTelNo(String telNo) {
		telNo = StringUtils.isEmpty(telNo) ? "" : telNo.trim();
	...
	public static String splitRegNo(String regNo) {
		if (StringUtils.isNotEmpty(regNo) && regNo.length() > 5) {
	...
	public static void setPageInfo(ZValue zvl, int showCnt) {
		int nowPage = (!zvl.containsKey("nowPage") || isEmpty(zvl.get("nowPage"))) ? 1 : zvl.getInt("nowPage");
		zvl.put("startNum", (nowPage - 1) * showCnt + 1);
		zvl.put("endNum", nowPage * showCnt);
		zvl.put("nowPage", nowPage);
	}
	...
	public static String numberGen(int len, int dupCd ) throws NoSuchAlgorithmException {
	 ...
	 */
	 public static String xssFilter(String str) {
		...
		str = xssFilter.doFilter(str);

		return str;
	}

	 public static String smartEditerXssFilter(String str) {
		 str = str.replaceAll("<\\/*(?:applet|b(?:ase|gsound|link)|embed|frame(?:set)?|i(?:frame|layer)|l(?:ayer|ink)|meta|object|s(?:cript|tyle)|title|xml|textarea|javascript|vbscript|background)[^>]*?>", "");
//		 XssFilter xssFilter = XssFilter.getInstance("lucy-xss-superset.xml");
//		 str = xssFilter.doFilter(str);
